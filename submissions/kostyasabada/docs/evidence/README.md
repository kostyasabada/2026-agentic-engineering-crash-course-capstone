...
## Layered server architecture decision

- Task: `add-realtime-chat-room-layered-arch`.
- User decision (2026-09-26, translated: "yes. and let's build it for extensibility with the proper controllers, services and repositories approach."): the server code uses a layered architecture with controllers, services, and repositories. The same message's "yes" answered the coordinator's question "add the import-boundary ESLint rule to task 1.3?": task 1.3 adds `no-restricted-imports` preventing `src/app/**` from importing `src/server/**`.
- Proposal (not yet a user decision at the time of round 1): the concrete feature-module layout with further ESLint layer boundaries is Proposal P23 in the change's design (D4, open question Q13).
- User decision (2026-09-26, translated: "yes, I accept both, commit 1.2"), after the coordinator explained P23 and recommended the controller boundary row: Proposal P23 and the controller boundary row are accepted; Q13 is resolved. "Commit 1.2" referred to the separate task 1.2 commit.
- User decision (2026-09-26, translated: "ok, that works"), after the coordinator explained the maker's additions and a coordinator addendum: an invalid `lastSeenId` or one greater than the highest message id is treated as absent (latest 100), service errors are acknowledged as `server_error` without a broadcast, ids are never reused, and `createdAt` is assigned by the service from an injected clock. The stale-`lastSeenId` rule is observable, so the chat-room spec gained one scenario.
